...
void ClosedLoopPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (!model_) {
    ROS_ERROR(
        "Parent model is NULL! GazeboNaoqiControlPlugin could not be loaded.");
    return;
  }

  // Check that ROS has been initialized
  if (!ros::isInitialized()) {
    ROS_ERROR("A ROS node for Gazebo has not been initialized, unable to load "
              "plugin.");
    return;
  }
  ...
  if (!_sdf->HasElement("joint")) {
    ROS_ERROR(
        "No joint element present. ClosedLoopPlugin could not be loaded.");
    return;
  }
  ...
  if (!_sdf->HasElement("child")) {
    ROS_ERROR(
        "No child element present. ClosedLoopPlugin could not be loaded.");
    return;
  }
  ...
  if (!_sdf->HasElement("parent")) {
    ROS_ERROR(
        "No parent element present. ClosedLoopPlugin could not be loaded.");
    return;
  }
  ...
  if (!child_) {
    ROS_ERROR("No Link named %s. ClosedLoopPlugin could not be loaded.",
              child_name_.c_str());
    return;
  }
  ...
  // we convert the strings into a string vector, spliting it at each space
  // using the function Split_String
  ...
  std::vector<float> rotations_splited_converted =
      Convert_to_float(rotations_splited);
  std::vector<float> positions_splited_converted =
      Convert_to_float(positions_splited);
  ...
  ignition::math::Pose3d jointOrigin = ignition::math::Pose3d(
      positions_splited_converted[0], positions_splited_converted[1],
      positions_splited_converted[2], rotations_splited_converted[0],
      rotations_splited_converted[1], rotations_splited_converted[2]);
...
ClosedLoopPlugin::Split_String(const std::string &subject) {
...
std::vector<float>
ClosedLoopPlugin::Convert_to_float(const std::vector<std::string> &subject) {
