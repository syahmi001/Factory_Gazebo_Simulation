...
// global variables
std::vector<int8_t>
    g_cylinder_active_pool; // the POOL, elements are index of cylinders
// add cylinder to pool from this index when checking x position
// avoid repeated adding, some cylinders may be grasped by scara and back in
// range
...
void cylinderPosesCallback(
  ...
  for (int i = cylinder_add_start_index; i < cylinder_quantity; i++) {
    if (current_cylinder_x[i] > RANGE_LOWER_LIMIT_R &&
  ...
  for (int i = 0;
       i < g_cylinder_active_pool.end() - g_cylinder_active_pool.begin(); i++) {
    // need use end()-begin(), because the vector length may change
    if (current_cylinder_x[g_cylinder_active_pool[i]] < RANGE_LOWER_LIMIT_R) {
...
bool cylinderPoolCallback(
    two_scara_collaboration::pool_claim_msgRequest &request,
    two_scara_collaboration::pool_claim_msgResponse &response) {
  // service callback for robot to claim cylinder from the active pool
  ROS_INFO_STREAM("");
  ROS_WARN(
      "in the /cylinder_pool_claim service"); // just to highlight the result
  ...
  if (response.cylinder_claimed == false)
    ROS_INFO_STREAM("cylinder " << requested_cylinder_index
                                << " fail to be claimed");
  else
