...
std::vector<double> scara_right_kinematics(std::vector<double> joints) {
  ...
  output_position[0] = -y_generic - 3;  // x coordinate
...
std::vector<double>
scara_right_inverse_kinematics(std::vector<double> position) {
...
void scaraleftUpperBoundaryCallback(
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber scara_right_r1_pos_subscriber = nh.subscribe(
      "/scara_robot_right_rotation1_pos", 1, scararightR1PosCallback);
  // initialize subscriber to "/scara_robot_right_rotation2_pos"
  ros::Subscriber scara_right_r2_pos_subscriber = nh.subscribe(
      "/scara_robot_right_rotation2_pos", 1, scararightR2PosCallback);
  // publish to topic "/scara_robot_right_rotation1_pos_cmd"
  ros::Publisher scara_right_r1_cmd_publisher = nh.advertise<std_msgs::Float64>(
      "/scara_robot_right_rotation1_pos_cmd", 1);
  std_msgs::Float64 scara_right_r1_cmd_msg;
  // publish to topic "/scara_robot_right_rotation2_pos_cmd"
  ros::Publisher scara_right_r2_cmd_publisher = nh.advertise<std_msgs::Float64>(
      "/scara_robot_right_rotation2_pos_cmd", 1);
  ...
  // subscribe to topic "/scara_left_upper_boundary"
  ros::Subscriber scara_left_upper_boundary_subscriber = nh.subscribe(
      "/scara_left_upper_boundary", 1, scaraleftUpperBoundaryCallback);
  // service client to service "/scara_right_upper_boundary_change"
  ...
  while (ros::ok()) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      scara_right_upper_boundary_client.call(
          right_boundary_change_srv); // call the service
      // delay and update
      ros::Duration(MOTION_SAMPLE_TIME).sleep();
      ros::spinOnce();
    }
