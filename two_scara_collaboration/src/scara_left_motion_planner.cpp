...
// only in motion step 1, the operation may be delayed when the other robot is
// in operation range
...
const double RANGE_UPPER_LIMIT_R = sqrt(pow(1.8, 2) - pow(1.5, 2)) - 3.0;
const double RANGE_LOWER_LIMIT_R = -sqrt(pow(1.8, 2) - pow(1.5, 2)) - 2.5;

const double RANGE_UPPER_LIMIT = sqrt(pow(1.8, 2) - pow(1.5, 2)) + 1.0;
const double RANGE_LOWER_LIMIT = -sqrt(pow(1.8, 2) - pow(1.5, 2)) + 1.5;

const double CYLINDER_SPEED =
    0.12;                         // the measured absolute speed of the cylinder
...
std::vector<double> scara_left_kinematics(std::vector<double> joints) {
  // x_generic and y_generic for the generic coordinates used in scara
  // kinematics
  ...
  // literally for the final placing place
  // position for dropping the picked cylinder
  output_position[0] = y_generic + 1;   // x coordinate
...
std::vector<double>
scara_left_inverse_kinematics(std::vector<double> position) {
...
void currentCylinderCallback(
    const std_msgs::Int8MultiArray &current_cylinder_msg) {
...
void cylinderPosesCallback(
    const two_scara_collaboration::cylinder_blocks_poses &cylinder_poses_msg) {
...
void scaraRightUpperBoundaryCallback(
    const two_scara_collaboration::scara_upper_boundary &upper_boundary_msg) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber current_cylinder_subscriber =
      nh.subscribe("/current_cylinder_blocks", 1, currentCylinderCallback);
  // initialize subscriber to "/cylinder_blocks_poses"
  ros::Subscriber cylinder_poses_subscriber =
      nh.subscribe("/cylinder_blocks_poses", 1, cylinderPosesCallback);
  // initialize subscriber to "/scara_robot_left_rotation1_pos"
  ros::Subscriber scara_left_r1_pos_subscriber = nh.subscribe(
      "/scara_robot_left_rotation1_pos", 1, scaraLeftR1PosCallback);
  // initialize subscriber to "/scara_robot_left_rotation2_pos"
  ros::Subscriber scara_left_r2_pos_subscriber = nh.subscribe(
      "/scara_robot_left_rotation2_pos", 1, scaraLeftR2PosCallback);
  // publish to topic "/scara_robot_left_rotation1_pos_cmd"
  ...
  // subscribe to topic "/cylinder_active_pool"
  ros::Subscriber cylinder_pool_subscriber =
      nh.subscribe("/cylinder_active_pool", 1, cylinderPoolCallback);
  // service client to service "/cylinder_pool_claim"
  ros::ServiceClient cylinder_pool_claim_client =
      nh.serviceClient<two_scara_collaboration::pool_claim_msg>(
          "/cylinder_pool_claim");
  two_scara_collaboration::pool_claim_msg cylinder_claim_srv_msg;
  // subscribe to topic "/scara_right_upper_boundary"
  ros::Subscriber scara_right_upper_boundary_subscriber = nh.subscribe(
      "/scara_right_upper_boundary", 1, scaraRightUpperBoundaryCallback);
  // service client to service "/scara_left_upper_boundary_change"
  ros::ServiceClient scara_left_upper_boundary_client =
      nh.serviceClient<two_scara_collaboration::upper_boundary_change>(
  ...
  std::vector<double>
      cylinder_drop_joints; // cylinder drop position in scara joints
  ...
  while (ros::ok()) {
    ...
    while (!cylinder_claimed) {
      ...
      for (int i = 0; i < g_cylinder_pool.size(); i++) {
        if (g_cylinder_y[g_cylinder_pool[i]] < -0.25) // can't reach
          continue;
        if ((g_cylinder_x[g_cylinder_pool[i]] > RANGE_LOWER_LIMIT) &&
            (g_cylinder_x[g_cylinder_pool[i]] < RANGE_UPPER_LIMIT)) {
          cylinder_claim_srv_msg.request.cylinder_index = g_cylinder_pool[i];
          // call the cylinder claim service
          if (cylinder_pool_claim_client.call(cylinder_claim_srv_msg)) {
            if (cylinder_claim_srv_msg.response.cylinder_claimed) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      //   if ((g_cylinder_x[g_cylinder_pool[i]] > RANGE_LOWER_LIMIT_R) &&
      //       (g_cylinder_x[g_cylinder_pool[i]] < RANGE_UPPER_LIMIT_R)) {
    ...
    // 2.target cylinder hovering (for cylinder grasping operation)
    // define durance for this step is 2 second, the grasping behaviros hasspens
    // at 0.5 second: lower the gripper 1.0 second: grasp the cylinder 1.5
    // second: lift the gripper
    ...
    ROS_INFO_STREAM("drop position: " << cylinder_drop_position[0] << ", "
                                      << cylinder_drop_position[1]);
    ROS_INFO_STREAM("drop joints: " << cylinder_drop_joints[0] << ", "
                                    << cylinder_drop_joints[1]);
    ...
    for (int i = 0; i < count; i++) {
      ...
      // scara_position_new[0] = ((count-i-1)*scara_position[0] +
      // cylinder_drop_position[0])/(count-i); scara_position_new[1] =
      // ((count-i-1)*scara_position[1] + cylinder_drop_position[1])/(count-i);
      // scara_joints_new = scara_left_inverse_kinematics(scara_position_new);
      ...
      scara_left_upper_boundary_client.call(
          left_boundary_change_srv); // call the service
      // delay and update
      ros::Duration(MOTION_SAMPLE_TIME).sleep();
      ros::spinOnce();
    }
    ...
    ROS_INFO_STREAM("stand by position: " << stand_by_position[0] << ", "
                                          << stand_by_position[1]);
    ROS_INFO_STREAM("stand by joints: " << stand_by_joints[0] << ", "
                                        << stand_by_joints[1]);
